                ...
                if (savedCiv) {
                    ...
                    fetch(`${api}/civilisations/${savedCiv}/units`)
                        ...
                        .then(units => {
                            ...
                            const groups = {
                                infantry: [], cavalry: [], champion: [], hero: [], ship: [], siege: [], support: [], other: []
                            };
...
function displayUnitDetails(clickedUnit, allCivUnits) {
    ...
    if (clickedUnit.hasMultipleVariants) {
        const basicVariant = allCivUnits.find(u => 
        ...
        if (basicVariant) {
            ...
            console.warn(`No 'Basic' rank variant found for ${clickedUnit.Identity?.SpecificName}. Displaying clicked unit.`);
        }
    }
    ...
    const addStat = (label, value, parentElement) => {
        ...
        if (!parentElement) {
            console.error('addStat called without a valid parentElement for label:', label);
            return;
        }
    ...
    if (unitToShow.Identity) {
        ...
        addStat('Classes', unitToShow.Identity.VisibleClasses || unitToShow.Identity.Classes, statsList);
    ...
    for (const key in unitToShow) {
        if (unitToShow.hasOwnProperty(key) && key !== 'Identity' && key !== 'hasMultipleVariants' && key !== 'code') {
